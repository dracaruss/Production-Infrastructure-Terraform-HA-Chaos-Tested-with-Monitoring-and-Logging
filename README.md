...
Ok for a second stress test, I decided to just overwhelm the CPUs themself.  
It's a more realistic simulation of what happens in production when an app is under heavy load.  
<br>
...
Ok now the run the test and see what happens:  
```
$ k6 run -e TARGET_URL=http://myapp-prod-alb-1305166777.us-east-2.elb.amazonaws.com stress_test.js
```
...
Also the instance count autoscaled up, and now once again was at 6:  
...
<br>
When the job is done, I'll be sending them .zip containing:  
* All the .tf files.  
* Documentation.  
